...
public static class ProjectEndpoints
{
    public static void MapProjectsEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/{ProjectId:guid}", ProjectGet)
            .WithName(nameof(ProjectGet))
            .Produces(StatusCodes.Status200OK);
    ...
    private static async Task<IResult> ProjectGet(
        [AsParameters] ProjectGetQuery query,
        [FromServices] IQueryHandler<ProjectGetQuery, LocalizrResponse<ProjectDetailsResponse>> handler)
    {
        var response = await handler.HandleAsync(query, CancellationToken.None);
        return Results.Ok(response);
    }
